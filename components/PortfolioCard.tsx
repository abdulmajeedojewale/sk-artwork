...
import { ExternalLink, Layers, Calendar, UserCheck } from 'lucide-react';
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-slate-900">
        ...
        <div className="absolute top-3 left-3 z-10">
          <span className="px-2.5 py-1 rounded-lg bg-amber-500/20 backdrop-blur-md border border-amber-500/40 text-amber-400 text-[11px] font-bold">
            {project.category?.name || 'Artwork Project'}
          </span>
        </div>

        <Link
          href={`/portfolio/${project.slug}`}
          className="absolute bottom-3 right-3 p-3 rounded-xl bg-amber-500 text-slate-950 opacity-0 group-hover:opacity-100 transition-all duration-300 shadow-lg shadow-amber-500/20"
